...
`modelName` 支持英文逗号分隔多个模型；聊天页切换会更新 `activeModelName`。`supportsVision` 控制是否先发多模态图片（默认 `true`，API 拒绝时自动去图重试）。厂商额外 HTTP 头写在 provider 的 `headers`（字面量或 `{{sessionId}}` / `{{providerId}}` / `{{model}}`）。详见 [`docs/使用文档.md`](./docs/使用文档.md#配置)。
