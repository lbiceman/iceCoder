...
> **已评测标记**：`01`、`02`（盲评归档）；`03`、`05`、`06`、`07`、`08`（Harness 增补，详见正式报告）
> **标记映射**：`01 = iceCoder`，`02 = CC`；后续批次均为 iceCoder Harness  
> **评测方式**：盲评（01/02）；Harness 复跑（03/05/06/07/08）  
> **出题模型**：GPT-5.5  
> **裁判模型**：GPT-5.5
> **归档说明**：`01/02` 与模型名称的映射为盲评完成后补充，仅用于结果归档；裁判评分阶段不可见。  
> **文档边界**：第 0-6 节为赛前任务规格 / 验收设计，第 7-9 节为 01/02 赛后摘要，第 9.1 节为后续批次对照；完整 Implementation Record 见正式报告。
...
后续 Harness 批次（03/05/06/07/08）已改变排序，见 §9.1；完整 Run 记录见正式报告。

---

## 9.1 后续批次对照（03 / 05 / 06 / 07 / 08）

完整 Gate / Judge / 探针证据见 [`../reports/debug-saas-order-supply-approval-fusion-05.md`](../reports/debug-saas-order-supply-approval-fusion-05.md)。本节只归档排名与对照表。

| 代号 | 模型 | SR | Composite | 等级 | Gate | Judge | Turns | Duration |
|------|------|-----|-----------|------|------|-------|-------|----------|
| **07** | union-alpha | 1 | **74** | B | 38 | 36 | 28 | ≈8m 7s |
| **05** | deeseek-v4.1-flash | 1 | **74** | B | 38 | 36 | 83 | 12m 23s |
| **08** | deepseek-v4.1-flash | 1 | **72** | B | 38 | 34 | 31 | ≈5m 36s |
| **03** | mimo-2.5 | 1 | **72** | B | 38 | 34 | — | — |
| **01** | — | 1 | **71** | B | 38 | 33 | 122 | ≈16m 8s |
| **06** | mimo-2.5 | 1 | **71** | B | 38 | 33 | — | ≈6m 40s |
| **02** | — | 1 | **69** | C | 38 | 31 | — | 4m 58s |
...
| 探针 | 07 | 05 | 08 | 03 | 06 | 01 | 02 |
|------|----|----|----|----|----|----|-----|
| `outboxCrossTenant` | 省略合并；传 tenant 可隔离 | 同 07 | 仅 aggregateId | 仅 aggregateId | 仅 aggregateId | 仅 aggregateId | 仅 aggregateId |
| `canShipTooMuch` | 通过 | 通过 | 通过 | 通过 | 通过 | 通过 | 失败 |
| `autoApproveAudit` | 基本通过 | 基本通过 | 直调 tenant 空 | 直调 tenant 空 | 直调 tenant 空 | tenant 空 | 双空 |

**07 vs 08**：同分档不成立。07 与 05 并列最佳（74）；08 与 03 同分（72），墙钟最短但隐藏语义退回 03/06 档。
...
本任务为 **L8 首发**。赛后 01/02 摘要见本文第 7–9 节；后续批次对照见 §9.1；完整 Implementation Record 见正式报告。
