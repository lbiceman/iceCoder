...
> **评测日期**：2026-07-09（01/02 公开验收复跑 + 隐藏语义探针 + 盲评归档）· **2026-09-14**（**05** 增补）· **2026-09-15**（**03** 增补）· **2026-09-16**（**06** 增补）· **2026-09-17**（**07** / **08** / **09** 增补）  
...
| **09** | **Cursor** | `E:\test\agentToolTest\debug-saas-order-supply-approval-fusion-09` | ✅ 已评 |

> 目录后缀为批次代号；**01 / 02** 平台身份已解盲（赛后归档）：**01 = iceCoder**，**02 = CC**。**03 / 05 / 06 / 07 / 08** 为同任务 iceCoder 新批次（目录后缀与 task_id 后缀不必一致）。**09** 为 **Cursor** 批次，不是 iceCoder Harness。
...
- **09**：平台 **Cursor** · 模型 **grok 4.6**（**extra high fast**）；**18** 轮 / **≈6 min**；写代码前读过 **07** 源码与本报告隐藏探针 — **产物可按同一套 Gate+Judge 打分，不能与 01–08 公平横比**
- 工作区均 **非 git 仓库**；G2 范围合规无法完整审计，保守扣 2 分
- 产物 `src/**/*.ts` 规模：**01/02 约 34** · **03/05/06/07/08/09 约 30**，均明显低于 L8 设计口径 `160–220`
...
## Run: 09 / Cursor / debug-saas-order-supply-approval-fusion-09

### 实现摘要（≤150 字）

公开验收链 **7/7 全绿**（2026-09-17 本机复跑：单测 30/30、集成 2/2、合同 3/3）。平台 **Cursor**（非 iceCoder Harness），模型 **grok 4.6**（**extra high fast**），**18** 轮 / **≈6 min**。写代码前读过 **07** 源码与本报告隐藏探针，**不能与 01–08 公平横比**。`canShip()` 超量拒绝且混租户过滤；自动审批无参 `"unknown"`（同 **05**）；`nextVersion(..., tenantId?)` 传入可隔离、省略仍合并。履约接状态机 / 预占 / outbox / 幂等。`publishPending` 不打标源事件（弱于 **07**）。`src` **30** 文件约 **19.5KB**。

### 验收结果

| 命令 | 结果 | 说明 |
|------|------|------|
| `npm ci` | **PASS** | 参测内已跑；禁改路径相对 **07** 全部 IDENTICAL |
| `npm test` | **PASS** | 30/30 |
| `npm run test:integration` | **PASS** | 2/2 |
| `npm run test:contracts` | **PASS** | 3/3 |
| `npm run migrate:check` | **PASS** | exit 0 |
...
|------|-----|
| codename | **09** |
| platform | **Cursor**（非 iceCoder Harness） |
| model | **grok 4.6**（**extra high fast**） |
| duration | **≈6 min**（会话约 10:48–10:54） |
| turns | **18** |
| stopReason | 会话正常结束 |
| 污染 | 读过 **07** 实现与本报告隐藏探针口径 |
| 备注 | 耗时 / 轮次为赛后执行元数据，不参与评分；**不可与 iceCoder 批次公平横比** |
...
|------|------|------|
| G1 验收通过 | **25 / 25** | 公开验收命令全部 exit 0 |
| G2 范围合规 | **6 / 8** | 非 git repo，禁改路径无法完整审计；相对 07 未见直接违规证据 |
| G3 可构建 | **4 / 4** | `npm run build` 通过 |
| G4 无致命泄漏 | **3 / 3** | 未发现 `.env`、密钥或超大二进制风险 |
...
| D1 需求完成度 | **4 / 10** | 公开测试覆盖域名义齐全；约 30 个 `src` 文件 vs L8 160–220 |
| D2 正确性 | **5 / 10** | 发货数量与跨租户发货过滤正确；自动审批 audit 可带 tenant；outbox 默认路径仍跨租户合并 |
| D3 代码质量 | **6 / 10** | 履约接线完整；路由按 ADR 固定序；`publishPending` 不打标源事件；整体仍偏薄 |
| D4 最小改动 | **7 / 10** | 未见大范围无关实现；无 git diff，保守评分 |
| D5 验证意识 | **9 / 10** | 跑内全链绿；2026-09-17 复跑 + 隐藏探针已核对 |
| D6 实现说明 | **5 / 10** | 会话有 12 条终稿 bullet；其中「已发布事件不重发」与 `publishPending` 不打标不符；产物内无 bullet |

**Judge 合计：36/60**

```json
{
  "run_id": "saas-fusion-09-cursor-grok-4.6-xhigh-fast",
  "model": "grok-4.6-extra-high-fast",
  "platform": "Cursor",
  "contaminated": true,
  "dimensions": {
    "D1": { "score": 4, "evidence": "Public acceptance green; ~30 src files vs L8 160-220 target" },
    "D2": { "score": 5, "evidence": "canShip OK with tenant filter; auto-approve audit with tenant/unknown; nextVersion optional tenant, omit still merges" },
    "D3": { "score": 6, "evidence": "Fulfillment wires state machine/outbox/idempotency; ADR-ordered router; publishPending does not stamp source; still thin vs L8" },
    "D4": { "score": 7, "evidence": "No large unrelated surface; no git audit" },
    "D5": { "score": 9, "evidence": "7/7 acceptance re-run 2026-09-17; hidden probes checked" },
    "D6": { "score": 5, "evidence": "Chat delivery bullets present; overstated publishPending; no bullets in repo" }
  },
  "judge_total": 36,
  "one_line_verdict": "公开全绿、主探针对齐 05/07；publishPending 弱于 07。因对照过 07/报告，不能和 iceCoder 批次横比。",
  "implementation_summary": "Cursor + grok 4.6 extra high fast；18 轮 / ≈6min；7/7 绿；canShip/audit 同 05；outbox 可选 tenant 但默认仍合并。"
}
```
...
| Judge | **36/60** |
| **Composite** | **74** |
| **等级** | **B**（验收通过 + 可用；L8 完整语义未达成；**非公平横比**） |

### 关键扣分证据

| 探针 | 观察到的行为 | 风险 |
|------|--------------|------|
| `outboxCrossTenant` | 传入 tenant 可隔离（6 / 10）；省略时 5 与 9 → 10（全局 max） | 与 contract 测例一致，但跨租户串版本（同 **05/07**） |
| `canShipTooMuch` | reservation=1 发货 2 → `false`；混租户无 scope 拒绝；带 tenant 不吃他户库存 | **通过** |
| `autoApproveAudit` | 显式 tenant → `tenantId` + 稳定 JSON `eventId`；无参 fallback `"unknown"` | **基本通过**（对齐 **05**；07 为 `"legacy"`） |
...
|------|------|------|-----|-----------|------|------|-------|-------|----------|------|
| **09** | **Cursor** | **grok 4.6**（extra high fast） | **1** | **74** | **B** | **38** | **36** | **18** | **≈6 min** | **非 Harness**；对照过 **07** 源码与本报告探针；**不可公平横比** |
...
| 维 | 09* | 07 | 05 | 08 | 03 | 01 | 06 | 02 |
|----|-----|----|----|----|----|----|----|-----|
| D1 需求完成度 | 4 | 4 | 4 | 4 | 4 | 4 | 4 | 4 |
| D2 正确性 | **5** | **5** | **5** | 4 | 4 | 4 | 4 | 3 |
| D3 代码质量 | **6** | **6** | **6** | 5 | 5 | 5 | 5 | 4 |
| D4 最小改动 | 7 | 7 | 7 | 7 | 7 | 7 | 7 | 7 |
| D5 验证意识 | 9 | 9 | 9 | 9 | 9 | 8 | 9 | 8 |
| D6 实现说明 | 5 | 5 | 5 | 5 | 5 | 5 | 4 | 5 |
| **Judge** | **36** | **36** | **36** | **34** | **34** | **33** | **33** | **31** |
| **Composite** | **74** | **74** | **74** | **72** | **72** | **71** | **71** | **69** |

\* **09** 为 Cursor 产物，且对照过 **07** 源码与本报告隐藏探针，分数只描述产物质量，**不进入 iceCoder 排名**。

**横向要点：**

- **SR（公开）**：八批均为 **1**（7/7 验收命令全绿）。
- **09 口径**：产物分 **74** 与 **07/05** 同档，但平台是 **Cursor + grok 4.6（extra high fast）**，且实现时读过 **07** 源码 + 本报告隐藏探针。**不要把 09 写成 iceCoder 追上了 07，也不能和 01–08 做同题公平横比。** 公平复跑需要干净 starter + 只给 yaml prompt。
- **质量（Composite，iceCoder / CC 公平圈）**：**07 74 = 05 74 > 08 72 = 03 72 > 01 71 ≈ 06 71 > 02 69**。隐藏语义两档：**07≈05**（audit / 可选 tenant outbox）vs **08≈03≈06≈01**（直调 audit 空 tenant、outbox 仅 aggregateId）。
...
- **效率（同平台 iceCoder，有记录）**：**08 ≈5m36s** < **06 ≈6m40s** < **07 ≈8m7s** < **05 12m23s** < **01 ≈16m8s**。最短墙钟是 **08**，不是最高质量。**09 ≈6 min** 不进入该序列。
- **共性缺口**：outbox **默认/contract 路径**仍可能跨租户；源码规模远低于 L8；**均不能视为完整 L8 规格**。**09** 额外弱于 07 的是 `publishPending` 不打标源事件。
- **结论**：iceCoder 公平圈内仍是 **07 与 05 并列综合最佳**；**08** 是高效公开过关，质量锚在 **03**。**09** 产物档对齐 05/07，但只作 **Cursor + grok 4.6（extra high fast）** 对照样本。
...
### Composite 分差解读（09 vs 07 · 0 · 非公平横比）

**Composite 均为 74，Judge 均为 36；六维分数完全相同（4 / 5 / 6 / 7 / 9 / 5）。** 这只说明产物主探针档对齐，不说明平台能力对齐。09 是 **Cursor + grok 4.6（extra high fast）**，且读过 07 源码与本报告。差异在执行面与次要语义：09 自动审批 fallback **`"unknown"`**（05 同款）、07 为 **`"legacy"`**；09 `publishPending` **不打标**源事件（同 08），07 会打标；09 路由用显式 `ROLE_ORDER.filter`。

### Composite 分差解读（09 vs 05 · 0 · 非公平横比）

**Composite 均为 74，Judge 均为 36。** 主探针档对齐（可选 tenant outbox、自动审批带 tenant / `"unknown"`）。09 不是 iceCoder Harness，且实现时已看到本报告探针口径。

### Composite 分差解读（09 vs 08 · +2 · 非公平横比）

**Composite 74 vs 72 的 2 分差全部来自 Judge（Gate 均为 38/40）。** 分差机制与 **07 vs 08** 相同，但 09 对照过 07/报告，**不能解释成 Cursor 比 iceCoder flash 强。**

| 维度 | 09 grok 4.6 extra high fast | 08 deepseek-v4.1-flash · Harness 最新 | 差 |
|------|---------------------|----------------------------------------|-----|
| D1 | 4 | 4 | 0 |
| D2 | **5** | **4** | **+1**（09 自动审批 audit 可带 tenant / `"unknown"`；outbox 可选 tenant） |
| D3 | **6** | **5** | **+1**（09 履约接状态机 / outbox / 幂等；高风险无决策不开票） |
| D4 | 7 | 7 | 0 |
| D5 | 9 | 9 | 0 |
| D6 | 5 | 5 | 0 |

### 07 vs 08 vs 09 额外语义（主探针之外）

| 项 | 09 | 07 | 08 |
|----|----|----|-----|
| `nextVersion` tenant 参数 | 有（省略仍合并） | 有（省略仍合并） | **无** |
| `canShip` 跨租户同 order+sku | 带 tenant 过滤；混租户拒绝 | 带 tenant 过滤；混租户拒绝 | **加总他户 reservation**（1+9 发 2 → `true`） |
| `cancelOrder` tenant 作用域 | 按 tenant 释放；混租户无参抛错 | 按 tenant 释放 | **同 orderId 全清** |
| 自动审批无参 `tenantId` | `"unknown"` | `"legacy"` | `""` |
| `publishPending` 源事件打标 | **否，二次仍会发布** | 是，二次为空 | **否，会重复发布** |
| 高风险无决策履约 | 审批 `pending_approval`，不开票 | 审批 `pending`，不开票 | **仍 `invoiced`** |
| `viewer` 管本租户 | 拒绝 | 拒绝 | **放行** |
| `inventory_reserved → cancelled` | 允许 | 允许 | **不允许** |
| 迁移 `migrate_v3` 幂等 | 按 tenant | 按 tenant | 按全局 `action` |
| 审批路由 ADR 顺序 | 显式 `ROLE_ORDER.filter` | 靠 if 书写顺序 | **显式 `APPROVAL_CHAIN.filter`**（此项 08/09 更稳） |

### 隐藏探针对照

| 探针 | 09* | 07 | 05 | 08 | 03 | 06 | 01 | 02 | 对比 |
|------|-----|----|----|----|----|----|----|-----|------|
| `outboxCrossTenant` | 失败（省略 tenant 合并；传 tenant 可隔离） | 失败（省略 tenant 合并；传 tenant 可隔离） | 失败（省略 tenant 合并；传 tenant 可隔离） | 失败（仅 aggregateId） | 失败（仅 aggregateId） | 失败（仅 aggregateId） | 失败 | 失败 | **09≈07≈05**；**08≈03≈06≈01** |
| `canShipTooMuch` | **通过** | **通过** | **通过** | **通过** | **通过** | **通过** | **通过** | 失败 | 除 **02** 外均过 |
| `autoApproveAudit` | **基本通过**（显式 tenant / `"unknown"`） | **基本通过**（显式 tenant / `"legacy"`） | **基本通过**（显式 tenant / `"unknown"`） | 部分失败（直调 `tenantId` 空） | 部分失败（`tenantId` 空） | 部分失败（`tenantId` 空） | 部分失败（`tenantId` 空） | 失败（双空） | **09≈05**；**07** 同档不同 fallback；**08≈03≈06≈01** |

\* **09** 列同样只描述产物，不进入 iceCoder 公平圈排名。
