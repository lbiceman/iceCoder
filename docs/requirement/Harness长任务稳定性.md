...
> **收尾（2026-09-16）**：从 goal 编译 required 清单的 `TaskAcceptanceTracker` / `CompletionGate` 已删除，改为 D′ 停时验收。现行说明见 [`../harness/收尾策略-模型停手与验收门控.md`](../harness/收尾策略-模型停手与验收门控.md)。下文 §2 / §6 里点名 `task-acceptance-tracker` 的条目视为已由 D′ 替代，不要按旧文件改回去。  
