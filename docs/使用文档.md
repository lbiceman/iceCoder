...
| 工作台 · 会话工具 | `#/chat` 底栏「工具」 | ![会话工具](./assets/desktop-workbench-tools.png) |
| 工作台 · 变更文件 | `#/chat` 底栏「文件」 | ![变更文件](./assets/desktop-work-snapshot.png) |
| 空会话欢迎页 | `#/chat`（未发过话） | ![欢迎页](./assets/desktop-welcome.png) |
| 工作 / 聊天（浅色） | `#/chat` | ![浅色工作聊天](./assets/desktop-work-chat-light.png) |
| 记忆图谱 | `#/memory` | ![记忆图谱](./assets/desktop-memory-graph.png) |
| 技能库 | `#/skills` | ![技能库](./assets/desktop-skills.png) |
| 统计 | `#/stats` | ![统计](./assets/desktop-stats.png) |
| 统计（浅色） | `#/stats` | ![浅色统计](./assets/desktop-stats-light.png) |
...
`data/config.json` 示例（默认按 OpenAI 兼容协议调用；Anthropic 原生协议见下文）：
...
默认把所有 provider 当成 OpenAI 兼容（`/v1/chat/completions`）。要接 Claude 官方 Messages API，需要单独的请求体、`x-api-key` 鉴权和 SSE 事件解析，不能走 OpenAI 适配器。

| 字段 / 机制 | 说明 |
|-------------|------|
| `apiMode` | 填 `anthropic_messages` |
| `apiUrl` | 官方一般为 `https://api.anthropic.com` 或 `https://api.anthropic.com/v1`（适配器会拼到 `/v1/messages`） |
| 自动识别 | 主机名为 `api.anthropic.com`（及 `*.anthropic.com`）时，即使未填 `apiMode` 也会走 Anthropic 适配器 |
| 鉴权 | 请求头 `x-api-key` + `anthropic-version: 2023-06-01`（**不**发 `Authorization: Bearer`） |
| 自定义网关 | 非官方域名必须显式设置 `apiMode: "anthropic_messages"` |
...
密钥可留空，改用环境变量 `ANTHROPIC_API_KEY` 或 `{PROVIDER_ID}_API_KEY`（例如 `id` 为 `anthropic` 时是 `ANTHROPIC_API_KEY`）。

### 额外请求头（headers）

部分兼容网关会要求自定义 HTTP 头（例如 OpenCode Go 的 `x-opencode-session`）。写在对应 provider 的 `headers` 上，**不要改源码**：

| 写法 | 发出去的值 |
|------|------------|
| `"x-client": "iceCoder"` | 字面量 `iceCoder` |
| `"x-opencode-session": "{{sessionId}}"` | 当前会话 ID |
| `"x-opencode-client": "iceCoder"` | 字面量 |
| `{{providerId}}` / `{{model}}` | 配置里的 provider `id` / 当前模型名 |

未知 `{{foo}}` 会在保存配置时直接报错。不可覆盖 `Authorization`、`Content-Type` 等保留头。设置页「额外请求头」按每行 `Header: value` 编辑。
...
环境变量（`ICE_DATA_DIR`、`ICE_SKILLS_DIR`、`ICE_SUPERVISOR_CONFIG_PATH` 等）见 [`环境变量.md`](./环境变量.md)。监管档位写在 `data/config.json` 的 **`supervisorMode`**（`off` / `adaptive` / `strict`），不是环境变量。
...
```bash
npm run build          # patch 版本 +1，然后 tsc + vite + npm pack
npm run build:install  # 上述 + 全局安装（新版本覆盖旧 iceCoder）
...
产物：`desktop/release/iceCoder-windows.exe`，并复制到 `releases/windows/`。CLI tgz 经 `npm run build` 复制到 `releases/npm/ice-coder.tgz`。
...
| `~supervisor` | Execution Mode 进入/退出遥测（`days=`；旧 L2 Timeline 已退役） |
| `~memory` | 记忆相关状态 |
...
npm run eval:agent -- --mode=local              # 只跑 scriptedTurns：隔离目录 + 真实改文件 / npm test
npm run eval:agent -- --mode=mock               # 无 API 烟测；带 scriptedTurns 的本地用例仍真实跑
...
目录在 `scripts/agent-eval-cases.ts`：原有编辑/压缩/记忆 case，加上 `stop-verification` 本地改文件用例（改已有文件、新建文件、测后再改、无计划、默认/显式失败、双命令、用户命令覆盖默认测试、验收自身改工作区、只读）。带 `scriptedTurns` 的用例必须用文件内容和命令 exit code 判定，mock 指标不得直接判过。收尾策略见 [`harness/收尾策略-模型停手与验收门控.md`](./harness/收尾策略-模型停手与验收门控.md)。
...
| `npm run eval:agent` | Agent 行为回归（`--mode=local` 真实改文件；`--mode=mock` 烟测） |
| `npm run telemetry:runtime` | 汇总 runtime 遥测 |
| `npm run build` | 自增 patch 版本并打 tgz 包 |
| `npm run install:global` | 把当前 tgz 装到全局（`--force`） |
| `npm run build:install` | 打包并覆盖全局 `iceCoder` |
