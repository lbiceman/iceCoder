...
npm run smoke:node-pty # 验证 node-pty 可 spawn PTY（/shell 协作依赖，见需求 §10）
```

**原生依赖**：`/shell` 交互协作使用 [`node-pty`](https://www.npmjs.com/package/node-pty)（Windows ConPTY / Unix PTY）。桌面打包时 `build:desktop:server` 会自动将 `server-bundle` 内的 `node-pty` 重编译为 Electron 内置 Node ABI（`desktop/scripts/rebuild-server-native.cjs`）；**Windows 需安装 Visual Studio「使用 C++ 的桌面开发」工作负载**。
...
| `npm run smoke:node-pty` | 验证 node-pty PTY spawn（/shell 依赖） |
