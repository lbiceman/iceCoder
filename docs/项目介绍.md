...
[English project guide](./PROJECT-GUIDE.md) | [环境变量](./环境变量.md) | [记忆系统详解](./记忆系统详解.md) 
...
| **双模 Supervisor** | **L0/L1/L3 单轴**（原 L2 takeover 栈已于 2026-08-31 删除）；总览 [`docs/双模机制详解.md`](./双模机制详解.md) |
| **记忆系统** | 文件化长期记忆 + 会话笔记 + Dream/淘汰；**24** 测试文件 **~446** 用例 — **§7** · [`记忆系统详解.md`](./记忆系统详解.md) |
...
| **iceCoder 工作台** | 右侧无 Tab 单栏：上检查点章节目录（标题右侧回滚）+ 下选中章执行流；底栏可弹出变更文件与会话工具名。编年史从会话数据装配。规格 [`requirement/工作台融合-最终效果-finish.md`](./requirement/工作台融合-最终效果-finish.md) |
...
**测试基线（2026-07-09）：** `test/**/*.test.ts` 共 **~225** 个文件、**~2,000** 条用例（`npm test`）。完整分层与手工结果见下文 **§10 测试与验收**。
...
| `src/llm/*` | OpenAI 兼容适配器 + Anthropic Messages 适配器 |
...
| `src/harness/task-domain.ts` | TaskIntent → `TaskDomain`（关键域 / 建图门禁） |
| `src/harness/supervisor/*` | 单轴监管：全局策略、ModeDecisionEngine、ToolGate、风险分类 |
| `src/types/supervisor.ts` | Supervisor 配置与 `GlobalModePolicy` 类型 |
| `data/supervisor-config.example.json` | 监管参数模板（`mode`、`executionMode`） |
...
| D′ 停时验收 | 无工具正文即提出结束；过期验收经 `run_command` 跑一次计划，只续一轮 |
...
`src/harness/sub-agent-runner.ts` 提供**隔离的只读子代理执行器**，由异步 `request_analysis` 后台任务调用。子代理使用白名单工具集（仅只读搜索/读取/列目录类工具），读取文件、搜索代码后写入 Analysis Workspace，并通过 **Analysis Ready** 摘要回流主上下文。
...
- `request_analysis` — 暴露给模型的异步后台分析工具
- `AnalysisSupervisor` / `analysis-ready-injector` — 调度后台分析并将结果注入主会话
...
### 3.6 单轴运行时监管（Supervisor）

机制详解：[`docs/双模机制详解.md`](./双模机制详解.md) · L2 退役说明：[`docs/L2监管层详解.md`](./L2监管层详解.md) · Gate 工作逻辑：[`docs/harness/Harness-L2与Gate工作逻辑.md`](./harness/Harness-L2与Gate工作逻辑.md)

> **2026-08-31：** 原 L2 Runtime Supervisor（takeover / PassiveObserver / CorrectionPort / EventTimeline）已删除。历史规格 [`docs/requirement/双模方案2-finish.md`](./requirement/双模方案2-finish.md) 仅供演进追溯。

当前把「用户选的监管档位」与「运行时执行约束」收敛为 L0 + L1 + L3：
...
| **L1** | `task-graph-executor.ts` · `task-domain.ts` | 结构化上下文注入；关键域建图门禁 |
| **L3** | `task-graph-review.ts` · GraphExecutor | 硬 block；fallback 分支切换；无 fallback → `user_checkpoint` |

**三档行为（L0）**

| 档位 | `supervisorMode` | 行为概要 |
|------|------------------|----------|
| **off** | `off` | 不启用 ModeDecisionEngine；不首轮建图 |
| **adaptive** | `adaptive`（默认） | 默认 free，结构性信号后进 forced；不首轮建图 |
| **strict** | `strict` | forced floor；关键工程任务首轮建图 |

**配置与入口**

- **档位**：`data/config.json` → `supervisorMode`（Web 顶栏 · `PATCH /api/config/supervisor-mode`）
- **细参数**：`data/supervisor-config.json`（示例 [`supervisor-config.example.json`](../data/supervisor-config.example.json)；仅 `mode` + `executionMode`）
- **加载**：`loadHarnessSupervisorRuntime()` — `chat` / `run` / `chat-ws` / `remote-ws`
- **环境变量**：见 [`docs/环境变量.md`](./环境变量.md) §4（`ICE_SUPERVISOR_SHADOW` 已随 L2 退役）

#### `~supervisor` 命令（Execution Mode 遥测）

Web 聊天输入框支持 **`~supervisor`**（输入 `~` 可打开命令面板补全），汇总 **Execution Mode** 进入/退出记录，数据源与 `GET /api/supervisor/events` 一致（`data/runtime/telemetry.jsonl`）。旧 L2 Timeline / `supervisor-events.jsonl` 不再读取。

| 参数 | 说明 | 默认 |
|------|------|------|
| （无参） | 最近 7 天的 Execution Mode 统计 | — |
| `days=N` | 统计最近 N 天（**1–90**） | `7` |
...
**HTTP 接口：** `GET /api/supervisor/events?days=7` — 返回 `executionMode.enter` / `exit` / `recent`（仅 `execution_mode_enter` / `execution_mode_exit`）。
...
| 类型 | 文件数 | 用例数（约） | 覆盖 |
|------|--------|-------------|------|
| 单元 / 集成 | **24** | **~446** | 召回、提取、Dream、淘汰、安全、并发、FactIndex |
| 端到端 | 含于上表 | `memory-e2e.test.ts` **9** 条 | 提取→写入→召回全链路 |

常用命令：`npm test -- test/memory/` · 设计说明：[`docs/记忆系统详解.md`](./记忆系统详解.md) · 变更记录：[`docs/requirement/记忆系统调整-finish.md`](./requirement/记忆系统调整-finish.md)
...
压缩器分层工作（深读：[`docs/压缩机制详解.md`](./压缩机制详解.md)）：
...
水位线（相对上下文窗口）：约 **80%** 微压缩 · **88%** 硬压缩 · 剩余 **&lt;18K** 亦硬压 · **93%** 激进收缩。
...
| Vite 开发服务器（`vite.config.ts` / `npm run dev -- --web`） | **1025** | 开发态 UI；`/api` 与 WS 代理到 `localhost:1024` |
...
![工作聊天 — 检查点目录与选中章执行流](./assets/desktop-work-chat.png)

![工作台 — 会话工具名单](./assets/desktop-workbench-tools.png)

![工作台 — 变更文件名单](./assets/desktop-work-snapshot.png)

![空会话欢迎页 — IceCoder 已就绪（深色）](./assets/desktop-welcome.png)

![空会话欢迎页 — IceCoder 已就绪（浅色）](./assets/desktop-welcome-light.png)

![工作聊天 — 浅色主题](./assets/desktop-work-chat-light.png)

![记忆图谱 — 标签筛选与节点关系图](./assets/desktop-memory-graph.png)

![技能库 — 列表与 Markdown 预览](./assets/desktop-skills.png)

![统计 — Token、记忆与监管趋势（深色）](./assets/desktop-stats.png)

![统计 — 浅色主题](./assets/desktop-stats-light.png)
...
附件上传/图片粘贴仍由 `chat-file.js` 经 `/api/chat/upload` 处理；vision 与 `image_read` 路径见下文「LLM 提供者与多模型」。

### LLM 提供者与多模型

`data/config.json` 的 `providers[]` 描述 LLM 端点（OpenAI 兼容或 Anthropic `/v1/messages`，与 MCP 配置分离）。

| 字段 | 说明 |
|------|------|
| `modelName` | 单个或**逗号分隔**多个模型名 |
| `activeModelName` | 当前选中模型（聊天页底部切换会写回配置） |
| `supportsVision` | Provider 级 vision 开关，默认 `true` |
| `headers` | 可选额外 HTTP 头；字面量或 `{{sessionId}}` / `{{providerId}}` / `{{model}}` |

**图片：** 默认先发送多模态内容；API 拒绝时 `LLMAdapter` 调用 `vision-fallback` 去图重试并注明「当前模型不支持图片识别」。`supportsVision: false` 时不发图片块，粘贴图落盘 `imagesCache` 并引导 `image_read`。配置详解见 [`使用文档.md`](./使用文档.md#配置)。
...
- [`docs/记忆系统详解.md`](./记忆系统详解.md) — 记忆系统完整说明（架构 / 召回 / Dream / 配置）
- [`docs/压缩机制详解.md`](./压缩机制详解.md) — 分层压缩 + 结构化恢复
- [`docs/双模机制详解.md`](./双模机制详解.md) — L0 / L1 / L3 单轴总览（原 L2 已退役）
- [`docs/L2监管层详解.md`](./L2监管层详解.md) — 已退役 L2 迁移说明
- [`docs/requirement/任务图规划-finish.md`](./requirement/任务图规划-finish.md) — TaskGraph / StepGraph（核心已实现）
- [`docs/requirement/执行透明-finish.md`](./requirement/执行透明-finish.md) — 旧 ETL 提示词层（已由 TaskGraph 替代）
- [`docs/requirement/执行流-最终效果-finish.md`](./requirement/执行流-最终效果-finish.md) — 编年史数据含义（用户句切章）
- [`docs/requirement/工作台融合-最终效果-finish.md`](./requirement/工作台融合-最终效果-finish.md) — **现行**工作台 UI（无 Tab，对齐 2026-09-14 源码）
...
- [`docs/requirement/L2测试过程.md`](./requirement/L2测试过程.md) — **双模完整测试手册**（~2,000 自动化 + 15 项 Web 手工场景）
- [`docs/requirement/双模方案2-finish.md`](./requirement/双模方案2-finish.md) — 双模监管规格 **V1.3.7**（I10 forced min dwell、signal 优先级、`enteredBy`）
- [`docs/requirement/运行时后续优化.md`](./requirement/运行时后续优化.md) — Phase **5E**（benchmark / Learning / 验收自动化，**后续不做**）
...
> 手工步骤与可复制提示词：[`docs/requirement/L2测试过程.md`](./requirement/L2测试过程.md) · 双模 e2e：`test/e2e/dual-mode-scenarios.test.ts`

### 10.1 自动化测试（Vitest）

**基线（2026-07-09）：** **~225** 测试文件 · **~2,000** 用例 · `npx tsc --noEmit` 0 error

| 目录 | 文件 | 说明 |
|------|------|------|
| `test/harness/` | **90** | Harness 主循环、压缩、checkpoint、**supervisor 全链路**、Shell 双轨 |
| `test/memory/` | **24** | 文件化记忆召回 / 提取 / Dream / 淘汰 |
...
npm test -- test/memory/                             # 记忆（~446 用例）
# 将 data/config.json 的 supervisorMode 设为 off（或 adaptive）做 harness 零回归
...
环境：Web 聊天 · Windows · 模型 z-ai/glm-5.1 / minimax-m2.5 · 详见 [`docs/requirement/L2测试过程.md`](./requirement/L2测试过程.md)
...
| F 工具失败 | adaptive | ✅ L1 forced / circuit breaker（旧 L2 信号口径已不适用） |
| G off 对照 | off | ✅ |
| H 长会话压测 | adaptive | ✅ |

> **L2-1～L2-7 手工场景已作废**（2026-08-31 删除 takeover / Timeline）。现行验收以 L0/L1/L3 + `npm test` 为准；历史手册见 [`requirement/L2测试过程.md`](./requirement/L2测试过程.md)。

### 10.4 发版门禁

1. `npx tsc --noEmit` — 0 error  
2. `npm test` — 全绿（默认配置）  
3. `data/config.json` → `supervisorMode: off` — harness 相关套件零回归  
4. 改 `supervisor/` 后补跑 execution-mode 相关测试（旧 `supervisor-bridge` / `recovery-boundary` 若已删除则跳过）
...
1. ~~Memory v2 结构化分级~~ — **已完成**：层级 / 证据强度 / intent 过滤 / 冲突同轮只注入一侧；Eval `memory-conflict` 已固化。可选后续：Dream 合并同主题偏好、token/噪声指标量化
2. 压缩与会话笔记的进一步耦合（如压缩前后 token 统计、恢复上下文预算裁剪等）——**结构化 `icecoder-runtime` 快照已可写入 `session-notes.md`**
...
5. **双模运行时监管**：核心链路已接入；继续按 [`docs/requirement/双模方案2-finish.md`](./requirement/双模方案2-finish.md) 与 [`docs/L2监管层详解.md`](./L2监管层详解.md) 补齐验收、遥测与边界场景。
6. 在现有 **Tool Planner** 之上，加强按失败模式动态规划与恢复策略（长期形态见 **`docs/requirement/任务图规划-finish.md`**）。
7. 多 Agent 协同：主 Agent 通过 **`request_analysis`** 请求后台只读分析，子 Agent 结果经 Workspace 和 Analysis Ready 回流。
