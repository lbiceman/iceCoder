...
export class MultiLevelMemoryLoader {
  ...
  private syncInterval: number = 5 * 60 * 1000; // 5分钟缓存
  ...
  async loadAllLevels(): Promise<Record<MemoryLevel, MemoryHeader[]>> {
    const levels = [MemoryLevel.PROJECT, MemoryLevel.USER, MemoryLevel.DIRECTORY];
  ...
  formatMultiLevelManifest(memoriesByLevel: Record<MemoryLevel, MemoryHeader[]>): string {
    ...
    const levelNames: Record<MemoryLevel, string> = {
      [MemoryLevel.PROJECT]: '项目',
      [MemoryLevel.USER]: '用户',
      [MemoryLevel.DIRECTORY]: '目录',
    };

    Object.entries(memoriesByLevel).forEach(([level, memories]) => {
      if (memories.length === 0) return;
      result += `\n## ${levelNames[level as MemoryLevel] ?? level} 记忆\n\n`;
  ...
  async getMemoryWithFreshness(memoryPath: string): Promise<string> {
    try {
      ...
      return memoryFreshnessNote(stat.mtimeMs) + content;
    } catch {
      return '';
    }
  }
