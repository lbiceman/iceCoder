...
(() => {
  ...
  const SUPERVISOR_MODES = ['off', 'adaptive', 'strict'];
  const SUPERVISOR_LABELS = { off: '自由', adaptive: '自适应', strict: '严格' };
  let currentSupervisorMode = 'adaptive';
  ...
  function syncSupervisorModeToPet(showBubble) {
    const label = SUPERVISOR_LABELS[currentSupervisorMode] || currentSupervisorMode;
  ...
  function cycleSupervisorMode() {
    const idx = SUPERVISOR_MODES.indexOf(currentSupervisorMode);
    const next = SUPERVISOR_MODES[(idx + 1) % SUPERVISOR_MODES.length];
  ...
  const THEME_KEY_DESKTOP = 'ice-theme';
  const THEME_KEY_MOBILE = 'ice-theme-mobile';
  ...
  function detectShellForTheme() {
    ...
    if (hash.startsWith('#/m/')) return 'mobile';
  ...
  function createListenerHub() {
    ...
    return {
      ...
      notify(arg) {
        for (let i = 0; i < listeners.length; i++) listeners[i](arg);
      },
    };
  }

  const supervisorModeHub = createListenerHub();
  const themeChangeHub = createListenerHub();
  const connectionChangeHub = createListenerHub();
  ...
  function parseMobilePathRoute(pathname) {
    ...
    if (path.startsWith('/m/work/')) {
      const id = path.slice('/m/work/'.length).split('/')[0];
  ...
  function resolveEntryRoute() {
    const pathRoute = parseMobilePathRoute(window.location.pathname);
  ...
  function parseRoute(hash) {
    ...
    if (h.startsWith('#/m/')) {
      ...
      if (rest.startsWith('/work/')) {
        const id = rest.slice('/work/'.length).split('/')[0];
        if (id) return { shell: 'mobile', page: 'workChat', sessionId: decodeURIComponent(id) };
      }
  ...
  function navigate(page) {
    if (currentShell === 'mobile') {
      navigateMobile({ shell: 'mobile', page });
      return;
    }
  ...
  function renderPage(page, sessionId) {
    ...
    if (prevPage === 'settings' && page !== 'settings') {
      const leavingSettings = window.SettingsPage || window.ConfigPage;
    ...
    if (!entry.mounted) {
      if (page === 'settings') {
        const settingsPage = window.SettingsPage || window.ConfigPage;
      ...
      const activeSettingsPage = window.SettingsPage || window.ConfigPage;
  ...
  function init() {
    ...
    if (path === '/m/chat' && window.location.hash && window.location.hash.startsWith('#/m/')) {
      const legacyRoute = parseRoute(window.location.hash);
    ...
    const initialRoute = resolveEntryRoute();
  ...
  window.AppRouter = {
    ...
    navigate(page, opts) {
      ...
      if (currentShell === 'mobile') {
        if (usesMobilePathRouting()) {
          const newPath = mobilePageToPath(page, opts);
          pushMobilePath(newPath);
          handleRouteChange(parseMobilePathRoute(newPath) || { shell: 'mobile', page });
        } else {
          const hash = mobilePageToHash(page, opts);
          pushMobileHash(hash);
    ...
    navigateWorkChat: function (sessionId) {
      ...
      if (currentShell === 'mobile') {
        if (usesMobilePathRouting()) {
          const chatPath = `/m/work/${encodeURIComponent(sessionId)}`;
          pushMobilePath(chatPath);
          handleRouteChange(parseMobilePathRoute(chatPath));
        } else {
          const hash = `#/m/work/${encodeURIComponent(sessionId)}`;
...
export const AppShell = typeof window !== 'undefined' ? window.AppShell : undefined;
export const AppRouter = typeof window !== 'undefined' ? window.AppRouter : undefined;
