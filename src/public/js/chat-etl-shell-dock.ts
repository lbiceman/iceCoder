...
((global) => {
  ...
  let dockEl = null;
  let listEl = null;
  const items = new Map(); // taskId → { el, lastTask }
  let stopHandler = null;
  let taskRemovedHandler = null;
  ...
  function statusText(task, isStopping) {
    ...
    const newPart = task.newLines > 0 ? ` · +${task.newLines} lines` : '';
    return `running · ${task.elapsed}${newPart}`;
  }

  function displayLabel(task) {
    const raw = task.label || task.command || task.taskId || '';
    const text = String(raw).replace(/[<>]/g, '');
    return text.length > 48 ? `${text.slice(0, 45)}...` : text;
  }

  function itemClass(task) {
    let cls = 'etl-shell-item is-running';
  ...
  function renderItemEl(el, task) {
    ...
    let labelEl = el.querySelector('.etl-shell-label');
    let statusEl = el.querySelector('.etl-shell-status');
    if (!labelEl) {
      el.innerHTML =
    ...
    const existingBtn = el.querySelector('.etl-shell-stop');
  ...
  function upsertTask(task) {
    ...
    const entry = items.get(task.taskId);
    if (entry && entry.el) {
  ...
  global.EtlShellDock = {
    ...
    resetMount,
    setStopHandler,
    setTaskRemovedHandler,
    resetStopPending,
    isRunningTask: isRunningDockTask,
  };
})(typeof window !== 'undefined' ? window : globalThis);

export const EtlShellDock = typeof window !== 'undefined' ? window.EtlShellDock : undefined;
