...
export const ChatExecutionPlanBridge = (() => {
  ...
  let enabled = false;          // 服务端 feature flag
  let connectedKnown = false;   // connected 到达后，其 capability 判定为权威值
  let attached = false;         // 已经挂载过订阅
  let currentPlanId = null;     // 本地 plan 跟踪 ID（防错位）
  let lastSyncMs = 0;
  let syncGeneration = 0;       // REST 请求代次，旧响应不得覆盖较新会话
  /** exit_forced 后抑制 REST 重同步，直到新一轮 task_graph / plan init */
  let planFootDismissed = false;
  /** 新一轮用户输入已清空面板时，忽略 REST /plan 返回的上一轮计划。 */
  let restPlanSuppressed = false;
  /** 已应用默认展开策略的 session + plan，避免同一计划重连时重复收起。 */
  const defaultCollapseApplied = Object.create(null);
  let lastShowPanelPref = prefShowPanel();
  ...
  function restoreFlowFromStorage(sessionId) {
    const stored = loadStoredFlow(sessionId);
  ...
  function persistFlowForSession(sessionId, bridgeState) {
    ...
    if (!sessionId) return;
    const panel = typeof window.ChatExecutionPlan.getFlowSnapshot === 'function'
      ? window.ChatExecutionPlan.getFlowSnapshot()
      : null;
    if (!panel) return;
    const plan = typeof window.ChatExecutionPlan.getPlan === 'function'
      ? window.ChatExecutionPlan.getPlan()
      : null;
    const bridgePlanId = bridgeState && bridgeState.currentPlanId;
    const bridgeFootDismissed = bridgeState && bridgeState.planFootDismissed;
    const planId = bridgePlanId || currentPlanId || (plan && plan.planId) || null;
    const hasFlow = !!(planId
  ...
  function applyRestPlan(plan, stored) {
    ...
    if (!plan) {
      const live = window.ChatExecutionPlan.getPlan
        ? window.ChatExecutionPlan.getPlan()
        : null;
      if (!live && stored) {
        applyStoredFlowSnapshot(stored);
  ...
  function prefDefaultExpanded() {
    try {
      if (window.EtlPrefs && typeof window.EtlPrefs.getKey === 'function') {
        const v = window.EtlPrefs.getKey('panelDefaultExpanded');
  ...
  function ensurePanelVisible() {
    ...
    const collapseKey = getActiveSessionId() + '::' + (currentPlanId || '__no_plan__');
  ...
  function onPrefsChange() {
    try {
      const showPanel = prefShowPanel();
      const turnedOn = !lastShowPanelPref && showPanel;
  ...
  function tryHydrateFromStructuredCache() {
    try {
      ...
      const structured = window.ChatSession.getStructuredMessages();
      if (structured && structured.length) {
        const ui = typeof window.ChatSession.getMessages === 'function'
  ...
  function onConnected(data) {
    ...
    ensurePanelVisible();
    resyncShellDockUi();
    const stored = restoreFlowFromStorage(getActiveSessionId());
  ...
  function onStepInner(data) {
    ...
    if (step.type === 'task_graph_update' && step.plan) {
      ...
      if (window.ChatExecutionPlan) {
        ...
        window.ChatExecutionPlan.setPlan({ ...step.plan });
      }
    }
  ...
  function onSessionUpdated() {
    if (!enabled) return;
    ensurePanelVisible();
    const stored = loadStoredFlow(getActiveSessionId());
  ...
  function onSessionSwitched() {
    ...
    const stored = restoreFlowFromStorage(sessionId);
    if (!stored) {
      try {
  ...
  function fetchAndApply(pendingStored) {
    if (!enabled) return;
    const sessionId = getActiveSessionId();
    let stored = pendingStored || loadStoredFlow(sessionId);
    if (planFootDismissed && stored) {
      applyStoredFlowSnapshot(stored);
      ensurePanelVisible();
      resyncShellDockUi();
...
if (typeof window !== 'undefined') {
  window.ChatExecutionPlanBridge = ChatExecutionPlanBridge;
}
