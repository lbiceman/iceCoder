...
export const ChatStaircaseNav = (() => {

  const MESSAGE_THRESHOLD = 10;
  const MAX_COLLAPSED_LINES = 15;
  const POPOVER_HOVER_DELAY_MS = 120;
  const POPOVER_LEAVE_DELAY_MS = 200;

  let elNav = null;
  let elLines = null;
  let elPopover = null;
  let elPopoverList = null;
  let elMessages = null;
  ...
  function resolveLineGroupStep(userTurnCount) {
    let step = getLineGroupStep(userTurnCount);
  ...
  function renderLines() {
    ...
    const step = resolveLineGroupStep(userTurnsCache.length);
    const groups = buildLineGroups(userTurnsCache, step);
    ...
    for (let g = 0; g < groups.length; g++) {
      ((group, groupIndex) => {
        ...
        const widthLevel = (groupIndex % 4) + 1;
  ...
  function renderPopoverList() {
    ...
    for (let i = 0; i < userTurnsCache.length; i++) {
      ((turn, turnIndex) => {
  ...
  function applyActiveHighlight(activeIndex) {
    const lineBtns = elLines ? elLines.querySelectorAll('.chat-staircase-line') : [];
    for (let li = 0; li < lineBtns.length; li++) {
      const lineIdx = parseInt(lineBtns[li].getAttribute('data-msg-index') || '-1', 10);
      const nextLineIdx = li + 1 < lineBtns.length
    ...
    const popItems = elPopoverList ? elPopoverList.querySelectorAll('.chat-staircase-popover-item') : [];
    for (let pi = 0; pi < popItems.length; pi++) {
      const itemIdx = parseInt(popItems[pi].getAttribute('data-msg-index') || '-1', 10);
  ...
  function updateActiveHighlight() {
    ...
    if (activeIndex < 0 && userTurnsCache.length > 0) {
      const nearBottom = window.ChatUI
...
if (typeof window !== 'undefined') {
  window.ChatStaircaseNav = ChatStaircaseNav;
}
