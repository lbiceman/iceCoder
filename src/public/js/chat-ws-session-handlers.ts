...
export const ChatWsSessionHandlers = (() => {
  ...
  function bind(WS, ctx) {
    ...
    function syncActiveSessionFromServer(data) {
      ...
      const serverId = data.activeSessionId || data.sessionId;
      if (!serverId) return false;
      const clientId = Session.getActiveId ? Session.getActiveId() : 'default';
    ...
    function onConnected(data) {
      const skipHeavyFetch = ctx.shouldSkipWsConnectedHeavyFetch();
      const paintedFromSessionSync = syncActiveSessionFromServer(data || {});
      ...
      const clientSid = Session.getActiveId ? Session.getActiveId() : 'default';
      const serverSid = data && (data.activeSessionId || data.sessionId);
      ...
      const dockSid = Session.getActiveId ? Session.getActiveId() : 'default';
      const connectedSid = data && (data.activeSessionId || data.sessionId);
      ...
      const rt = data && data.runningTurn;
    ...
    function onSessionCleared(data) {
      ...
      const sid = data.sessionId || activeId;
    ...
    function onSessionUpdated(data) {
      ...
      if (data && data.reason === 'user_message') {
        if (Session.fetchAndMergeRemoteUserMessages) {
          Session.fetchAndMergeRemoteUserMessages((added) => {
            if (!added) return;
...
if (typeof window !== 'undefined') {
  window.ChatWsSessionHandlers = ChatWsSessionHandlers;
}
