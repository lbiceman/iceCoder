...
export const ChatWsStreamHandlers = (() => {
  ...
  function bind(WS, ctx) {
    ...
    function onStream(data) {
      ...
      const petRead = ctx.getSessionPet();
    ...
    function onResponse(data) {
      ...
      UI.finalizeStreamResponse(Session.getMessages(), Session.stripStatusTag);
      UI.clearReasoningStream();
      const msg = { role: 'agent', content: Session.stripStatusTag(data.content || '') };
      Session.appendMessage(msg);
      const pending = ctx.getPendingTurnTokenUsage();
      if (pending) {
    ...
    function onStep(data) {
      ...
      if (step.type === 'thinking') {
        ...
        const msgsThink = Session.getMessages();
        for (let mti = msgsThink.length - 1; mti >= 0; mti--) {
      ...
      if (step.type === 'tool_result' && step.toolName) {
        const fmtResult = window.ToolTraceFormat;
        const resultStatus = fmtResult
        ...
        if (fmtResult && step.toolName === 'run_command' && window.BgTaskChip && ctx.getElMessages()) {
          const checkInfo = fmtResult.parseCheckTaskResult(step.toolOutput);
        ...
        if (window.ToolDisplayHistory) {
          let diffSource = window.ToolDisplayHistory.extractDiffSource(
            step.toolName,
            step.toolOutput,
            step.toolArgs,
          );
    ...
    function onStatus(data) {
      ...
      if (!processing) {
        ...
        const skipRefreshAfterUserStop = get('userStopped');
    ...
    function onError(data) {
      ...
      ctx.endTransparencyTurnTimer();
      UI.finalizeStreamResponse(Session.getMessages(), Session.stripStatusTag);
      const msg = { role: 'agent', content: `[err] ${data.message}` };
      Session.appendMessage(msg);
      UI.appendMessageEl(msg, Session.stripStatusTag);
      Session.saveMessages();
      Pet.removeThinking(get('isStreaming'), WS.isProcessing());
...
if (typeof window !== 'undefined') {
  window.ChatWsStreamHandlers = ChatWsStreamHandlers;
}
