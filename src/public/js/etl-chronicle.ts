...
export const EtlChronicle = (() => {

  const PREVIEW_MAX = 72;
  const TOOL_PREVIEW_MAX = 40;
  ...
  const SEARCH_TOOLS = { glob: true, grep: true };
  ...
  function isSystemInjectedUserText(text) {
    const t = String(text || '');
    if (!t) return false;
    if (t.includes('[System: Skill File Guide]')) return true;
    if (t.includes('[System:') && t.includes('Skill')) return true;
    return false;
  }

  function isRealUiUser(msg) {
    if (!msg || msg.role !== 'user' || isAlsoNote(msg)) return false;
    const text = stripSkillWall(extractText(msg.content));
  ...
  function previewFromUser(msg) {
    const text = stripSkillWall(extractText(msg && msg.content));
  ...
  function formatToolPreview(toolName, args) {
    const target = extractToolTarget(toolName, args);
    if (target) return clamp(target, TOOL_PREVIEW_MAX);
    return '';
  }

  function inferToolIntent(toolName, target) {
    const hint = target ? `「${clamp(basename(target), 24)}」` : '';
  ...
  function makeTool(tc, extras) {
    extras = extras || {};
    const name = tc && (tc.name || tc.toolName) ? String(tc.name || tc.toolName) : '';
    const args = tc && (tc.arguments || tc.toolArgs || tc.args);
    ...
    const iteration = normalizeIter(extras.iteration != null ? extras.iteration : (tc && tc.iteration));
  ...
  function buildChapter(opts) {
    ...
    const allTools = [];
    for (let i = 0; i < rounds.length; i++) {
      const ts = rounds[i].tools || [];
      for (let j = 0; j < ts.length; j++) allTools.push(ts[j]);
    }
    const writePaths = collectWritePaths(allTools);
    const chapter = {
  ...
  function roundsFromAssistants(slice) {
    ...
    for (let a = 0; a < assistants.length; a++) {
      const msg = assistants[a];
      const calls = assistantToolCalls(msg);
      const tools = [];
      for (let t = 0; t < calls.length; t++) {
        tools.push(makeTool(calls[t]));
      }
      const isLast = a === assistants.length - 1;
      const isFinal = isLast && tools.length === 0;
      rounds.push({
        iteration: a + 1,
  ...
  function tracesFromUiMessages(uiMessages) {
    ...
    for (let i = 0; i < uiMessages.length; i++) {
      ...
      const iter = normalizeIter(m.iteration);
  ...
  function mergeTraceMaps(a, b) {
    ...
    function add(src) {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        for (let j = 0; j < list.length; j++) out[k].push(list[j]);
      }
    }
    add(a);
    add(b);
  ...
  function makeRound(tools, isFinal, iteration) {
    tools = tools || [];
    const it = normalizeIter(iteration) || 1;
    return {
      iteration: it,
      title: deriveRoundTitle(tools, isFinal, isFinal),
      status: roundStatusFromTools(tools, 'done'),
  ...
  function roundsFromIteratedTools(tools, addFinal) {
    ...
    for (i = 0; i < tools.length; i++) {
      const it = normalizeIter(tools[i].iteration);
      if (!it) {
    ...
    if (orphans.length) {
      const extraIter = order.length ? (order[order.length - 1] + 1) : 1;
  ...
  function roundsFromUiSlice(userMsg, slice, toolTraces) {
    ...
    const rawTraces = collectSliceTraces(userMsg, slice, toolTraces);
    const lastAgent = agents.length ? agents[agents.length - 1] : null;
    const lastKey = lastAgent && lastAgent.id ? String(lastAgent.id) : '';
    const lastHasTraces = !!(lastKey && toolTraces[lastKey] && toolTraces[lastKey].length);
    const addFinal = !!(lastAgent && msgHasText(lastAgent) && !lastHasTraces);
    if (tracesHaveIteration(rawTraces) && rawTraces.length) {
      return roundsFromIteratedTools(toolsFromTraceList(rawTraces), addFinal);
    }
    const rounds = [];
    const usedParents = Object.create(null);
    for (i = 0; i < agents.length; i++) {
      const msg = agents[i];
      const parentKey = msg.id ? String(msg.id) : '';
      const traces = parentKey && toolTraces[parentKey] ? toolTraces[parentKey] : [];
      if (parentKey) usedParents[parentKey] = true;
      const tools = toolsFromTraceList(traces);
    ...
    const userKey = userMsg && userMsg.id ? String(userMsg.id) : '';
    if (userKey && toolTraces[userKey] && !usedParents[userKey]) {
      const extra = toolsFromTraceList(toolTraces[userKey]);
  ...
  function flattenTools(rounds) {
    const out = [];
    for (let i = 0; i < rounds.length; i++) {
      const tools = rounds[i].tools || [];
      for (let j = 0; j < tools.length; j++) out.push(tools[j]);
    }
    return out;
  }
  ...
  function finalizeRoundOrder(rounds) {
    ...
    rounds.sort((a, b) => {
      ...
      if (!a.isFinal && b.isFinal) return -1;
      const ia = typeof a.iteration === 'number' ? a.iteration : 0;
      const ib = typeof b.iteration === 'number' ? b.iteration : 0;
      return ia - ib;
    });
    const seenWork = Object.create(null);
    let collision = false;
    let hasHigh = false;
    for (let i = 0; i < rounds.length; i++) {
      const it = typeof rounds[i].iteration === 'number' ? rounds[i].iteration : 1;
    ...
    if (!hasHigh || collision) {
      for (let j = 0; j < rounds.length; j++) rounds[j].iteration = j + 1;
    }
    return rounds;
  }
  ...
  function mergeRoundLists(primary, secondary) {
    ...
    const list = primary.map(cloneRound);
    const byIter = Object.create(null);
    ...
    const unmatchedNoIter = [];
    for (i = 0; i < secondary.length; i++) {
      const sec = secondary[i];
      if (sec.isFinal) continue;
      tools = sec.tools || [];
      const secIter = normalizeIter(sec.iteration);
      for (j = 0; j < tools.length; j++) {
        ...
        const destIter = normalizeIter(t.iteration) || secIter;
        ...
        if (destIter > 1) {
          const dest = insertWorkRound(list, byIter, makeRound([], false, destIter));
          dest.tools.push(t);
          byId[key] = t;
          indexRound(dest);
          continue;
    ...
    if (unmatchedNoIter.length) {
      const workN = countWorkRounds(list);
      const lastWork = lastWorkRound(list);
      let primaryToolN = 0;
      ...
      if (workN <= 1 && lastWork) {
        ...
        const prefixIter = isFinite(minIter) && minIter > 1 ? minIter - 1 : 1;
    ...
    const secFinal = secondary.length && secondary[secondary.length - 1].isFinal;
    const baseFinal = list.length && list[list.length - 1].isFinal;
    if (secFinal && !baseFinal) {
      const lw = lastWorkRound(list);
  ...
  function pickRicherRounds(structRounds, uiRounds) {
    const uiWork = countWorkRounds(uiRounds);
    const structWork = countWorkRounds(structRounds);
  ...
  function collectChapterUsers(uiMessages, checkpointEntries, structured) {
    const uiUsers = collectRealUiUsers(uiMessages);
    if (uiUsers.length) return uiUsers;
    const fromCp = usersFromCheckpoints(checkpointEntries);
  ...
  function findStructuredUserIndex(structured, uiUser, used) {
    const needle = normalizeForMatch(uiUser);
    let fallback = -1;
    for (let i = 0; i < structured.length; i++) {
      if (used[i] || !isRealStructuredUser(structured[i])) continue;
      if (fallback < 0) fallback = i;
      if (!needle) continue;
      const hay = normalizeForMatch(structured[i]);
      if (!hay) continue;
      if (hay === needle || hay.includes(needle) || needle.includes(hay)) return i;
    }
    return fallback;
  }
  ...
  function sliceStructuredBetween(structured, startIdx, nextIdx) {
    ...
    const end = nextIdx >= 0 ? nextIdx : structured.length;
    if (end < start) return [];
  ...
  function assemble(input) {
    input = input || {};
    const uiMessages = Array.isArray(input.uiMessages) ? input.uiMessages : [];
    const structured = Array.isArray(input.structured) ? input.structured : [];
    const uiUsers = collectChapterUsers(uiMessages, input.checkpointEntries, structured);
    const toolTraces = mergeTraceMaps(input.toolTraces, tracesFromUiMessages(uiMessages));
    const chapters = [];
    ...
    const usedStruct = Object.create(null);
    const structIndexes = [];
    for (let s = 0; s < uiUsers.length; s++) {
      const found = findStructuredUserIndex(structured, uiUsers[s], usedStruct);
      structIndexes.push(found);
      if (found >= 0) usedStruct[found] = true;
    }

    for (let n = 0; n < uiUsers.length; n++) {
      const ui = uiUsers[n];
      const structIdx = structIndexes[n];
      const nextStructIdx = n + 1 < structIndexes.length ? structIndexes[n + 1] : -1;
      const structUser = structIdx >= 0 ? structured[structIdx] : null;
      const structSlice = sliceStructuredBetween(structured, structIdx, nextStructIdx);
      const uiSlice = uiSliceAfterUser(uiMessages, ui);
      const rounds = pickRicherRounds(
        roundsFromAssistants(structSlice),
        roundsFromUiSlice(ui, uiSlice, toolTraces),
      );
      const isLast = n === uiUsers.length - 1;
      const complete = hasCompletedReply(uiSlice, structSlice, rounds);
      const isLastIncomplete = isLast && !complete;
      const messageId = (ui && ui.id) ? String(ui.id) : '';
      const cp = checkpointByMessageId(input.checkpointEntries, messageId);
      let startTs = msgTs(ui);
      if (startTs == null && cp && typeof cp.userMessageTime === 'number') startTs = cp.userMessageTime;
      const endTs = lastSliceTs(uiSlice, lastSliceTs(structSlice, null));
      let preview = previewFromUser(ui || structUser);
  ...
  function durationOf(start, end) {
    ...
    const finish = typeof end === 'number' ? end : start;
    return Math.max(0, finish - start);
  }

  function toolFromLiveRecord(rec) {
    const target = rec.target || rec.detail || '';
    const tool = {
    ...
    const iteration = normalizeIter(rec.iteration);
    if (iteration) tool.iteration = iteration;
    return tool;
  }
  ...
  function fromLive(input) {
    ...
    const toolRecords = Array.isArray(input.toolRecords) ? input.toolRecords : [];
    for (let i = 0; i < toolRecords.length; i++) {
      const raw = toolRecords[i];
      if (raw && raw.toolCallId) toolsById[raw.toolCallId] = raw;
    }
    const roundRecords = Array.isArray(input.roundRecords) ? input.roundRecords : [];
    const rounds = [];
    for (let r = 0; r < roundRecords.length; r++) {
      const rec = roundRecords[r] || {};
      const tools = [];
      const ids = Array.isArray(rec.toolCallIds) ? rec.toolCallIds : [];
      for (let k = 0; k < ids.length; k++) {
        if (toolsById[ids[k]]) tools.push(toolFromLiveRecord(toolsById[ids[k]]));
      }
      const isLast = r === roundRecords.length - 1;
      const isFinal = !!rec.isFinal || (isLast && !tools.length);
      rounds.push({
...
if (typeof window !== 'undefined') {
  window.EtlChronicle = EtlChronicle;
}
