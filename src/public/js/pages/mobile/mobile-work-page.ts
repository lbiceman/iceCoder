...
export const MobileWorkPage = (() => {
  ...
  function render(parentEl) {
    ...
    if (!mounted) {
      ...
      if (isRemoteEntry()) {
        ...
        const dashboard = parentEl.querySelector('#mobile-work-dashboard');
  ...
  function attachComposer() {
    ...
    const slot = rootEl.querySelector('#mobile-composer-slot');
    if (slot) window.MobileComposerHost.attachToSlot(slot);
  }

  function attachRemoteChat() {
    if (!rootEl || !window.MobileComposerHost) return;
    const core = rootEl.querySelector('#mobile-work-chat-core');
  ...
  function syncDashboard() {
    ...
    const dashboard = rootEl.querySelector('#mobile-work-dashboard');
  ...
  function syncChatActivity() {
    if (!rootEl || isRemoteEntry()) return;
    const scroll = rootEl.querySelector('.mobile-work-scroll');
    if (!scroll) return;
    let msgCount = 0;
    if (window.ChatSession && typeof window.ChatSession.getMessages === 'function') {
      msgCount = window.ChatSession.getMessages().length;
    }
    const busy = window.ChatPage && typeof window.ChatPage.isWorkloadActive === 'function'
      && window.ChatPage.isWorkloadActive();
    const hide = msgCount > 0 || busy;
...
if (typeof window !== 'undefined') {
  window.MobileWorkPage = MobileWorkPage;
}
